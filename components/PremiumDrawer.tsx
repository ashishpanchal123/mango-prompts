"use client";

import { useEffect, useState } from "react";
import type { Prompt } from "@/lib/types";

import { PREMIUM_ACCESS_PAYMENT_URL } from "@/lib/constants";

export default function PremiumDrawer() {
  const [isOpen, setIsOpen] = useState(false);

  useEffect(() => {
    const handleOpen = (e: CustomEvent<{ prompt?: Prompt }>) => {
      setIsOpen(true);
    };

    window.addEventListener("open-premium-drawer" as any, handleOpen);
    return () => window.removeEventListener("open-premium-drawer" as any, handleOpen);
  }, []);

  if (!isOpen) return null;

  const handlePay = () => {
    localStorage.setItem("premiumAccessPending", "true");
    localStorage.setItem("premiumUnlockType", "all_premium");
    window.location.href = PREMIUM_ACCESS_PAYMENT_URL;
  };

  return (
    <>
      {/* Backdrop */}
      <div 
        className="fixed inset-0 bg-black/60 backdrop-blur-sm z-40 transition-opacity"
        onClick={() => setIsOpen(false)}
      />
      
      {/* Drawer */}
      <div className="fixed inset-y-0 right-0 z-50 w-full sm:w-[400px] bg-[var(--bg)] border-l border-[var(--border)] shadow-2xl flex flex-col transform transition-transform duration-300">
        <div className="p-6 border-b border-[var(--border)] flex justify-between items-center bg-[var(--bg-card)]">
          <h2 className="text-xl font-semibold">Unlock All Premium Prompts</h2>
          <button 
            onClick={() => setIsOpen(false)}
            className="text-[var(--text-muted)] hover:text-[var(--text)] transition-colors p-2"
          >
            ✕
          </button>
        </div>
        
        <div className="p-6 flex-1 overflow-y-auto flex flex-col gap-6">
          <div className="text-center mt-2">
             <div className="text-4xl font-bold mb-3 text-[var(--mango-soft)]">₹25</div>
             <p className="text-[var(--text-primary)] text-lg font-medium">Get access to all premium prompts for ₹25 only.</p>
          </div>

          <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-card)] p-5 relative overflow-hidden mt-2">
             <div className="absolute top-0 right-0 px-3 py-1 bg-[var(--mango)] text-[#1a1100] text-[10px] font-bold tracking-wider rounded-bl-xl">
               PREMIUM
             </div>
             <h3 className="font-semibold text-[var(--text)] mb-4">Benefits:</h3>
             <ul className="space-y-3 text-sm text-[var(--text-secondary)]">
                <li className="flex items-start gap-2">
                  <svg className="w-5 h-5 text-green-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7"/></svg>
                  Unlock all premium prompt cards
                </li>
                <li className="flex items-start gap-2">
                  <svg className="w-5 h-5 text-green-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7"/></svg>
                  Copy premium prompts instantly
                </li>
                <li className="flex items-start gap-2">
                  <svg className="w-5 h-5 text-green-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7"/></svg>
                  One-time launch access
                </li>
                <li className="flex items-start gap-2">
                  <svg className="w-5 h-5 text-green-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7"/></svg>
                  Secure payment via Razorpay
                </li>
             </ul>
          </div>
          
          <div className="mt-auto pt-6 flex flex-col gap-3">
             <button 
               onClick={handlePay}
               className="w-full py-3.5 bg-[var(--mango)] text-[#1a1100] font-semibold rounded-xl hover:bg-[var(--mango-soft)] transition-colors"
             >
               Subscribe Now - ₹25
             </button>
             <button 
               onClick={() => setIsOpen(false)}
               className="w-full py-3 text-sm font-medium text-[var(--text-secondary)] hover:text-[var(--text)] transition-colors"
             >
               Maybe Later
             </button>
          </div>
        </div>
      </div>
    </>
  );
}
